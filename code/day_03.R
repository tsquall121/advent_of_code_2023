...
input_string <- paste(readLines("data/day_03_data.txt"), collapse = "")

matches <- unique(unlist(str_extract_all(input_string, "[^0-9.]")))

matrix_data <- matrix(
    data = str_split(input_string, "")[[1]],
    nrow = length(readLines("data/day_03_data.txt")),
    ncol = nchar(input_string) / length(readLines("data/day_03_data.txt")),
    byrow = TRUE
)
...
final_extracted(result_matrix) # We can see these matched numbers have duplications caused by the digit-level matching algorithm from last section. A symbol may match multiple digits in a number so that the number will be extracted for several times.
...
input_string <- "467..114.....*........35..633.......#...617*...........+.58...592...........755....$.*.....664.598.."
...
for (i in length(row_indices)) {
    row_indices[i + 1] - row_indices[i] <= 2
}
